	...
	jQuery('form#tg_reservation_form').submit(function() {
		...
		jQuery('form#tg_reservation_form .required_field').each(function() {
			if(jQuery.trim(jQuery(this).val()) == '') {
				...
				jQuery('#reponse_msg ul').append('<li class="error">Пожалуйста, заполните поле '+labelText+'</li>');
				hasError = true;
			} else if(jQuery(this).hasClass('email')) {
				var emailReg = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/;
				if(!emailReg.test(jQuery.trim(jQuery(this).val()))) {
					var labelText = jQuery(this).prev('label').text();
					jQuery('#reponse_msg ul').append('<li class="error">Пожалуйста, внимательно заполните поле '+labelText+'</li>');
					hasError = true;
				}
			}
		});
